...
3. 实例配置的参数说明如下：
   ...
   |存储类型|不同的存储类对应的实例规格的最大IOPS不同，当前支持本地SSD、本地NVMe、SSD云盘，具体以控制台为准。存储类型情况请参考 [PostgreSQL 产品规格](https://docs.jdcloud.com/cn/rds/postgresql-specifications)。|
   |规格|实例的CPU和内存，不同的规格对应不同的最大连接数和最大IOPS, 请参考 [PostgreSQL 产品规格](https://docs.jdcloud.com/cn/rds/postgresql-specifications)。|

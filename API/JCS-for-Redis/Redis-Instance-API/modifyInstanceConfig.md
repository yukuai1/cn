...
|**configDefaultValue**|String|False| |参数默认值|
|**configValueType**|String|False| |参数值类型，目前有int、string、outputBuffer、byteArray这四种|
|**configValueMin**|Integer|False| |参数值的最小值，在configValueType为int时有效|
|**configValueMax**|Integer|False| |参数值的最大值，在configValueType为int时有效|
|**configValueOptional**|String[]|False| |参数值的可选值，在configValueType为string或byteArray时有效|
|**configValueOutputBuffer**|String|False| |configValueType为outputBuffer时的参数值规则|

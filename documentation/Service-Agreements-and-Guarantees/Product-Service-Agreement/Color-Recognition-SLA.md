本服务等级协议（Service Level Agreement，简称 "SLA"）规定了京东云向客户提供的 **颜色识别** 服务（以下简称"该服务"）的可用性等级指标及赔偿方案，向您承诺提供数据管理和业务质量方面的保障。同时，京东云有权根据变化适时对部分指标作出调整。若无特殊约定，本条款中的"年"均按自然年计算。
...
该服务是京东云开发的颜色识别服务。目前版本包括构建知识图谱过程中对文本数据中三元组进行结构化解析等功能。所有功能均已在官网提供详细的功能介绍和使用说明文档。
...
用户免费使用颜色识别服务的部分，不予赔偿。

对于用户付费使用颜色识别服务的部分，根据颜色识别服务可用性，按如下方案进行赔偿：
...
赔偿调用次数不超过用户在服务月度内已购买颜色识别服务次数包的次数总额。

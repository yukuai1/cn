...
#### 1. 进入实例列表页
当前页面会显示当前区域下的各个 ClickHouse实例的概要信息，包括计费信息和过期时间等。

点击 **【创建】** 按钮，新建一个 ClickHouse 实例。
...
#### 4. 网络，可用区及实例名称
1）分析型云数据库ClickHouse只支持在私有网络中创建。如果用户没有私有网络及子网，可以通过 **"新建私有网络”**和 **“新建子网”** 的链接创建私有网络和子网。创建完成后，点击**“刷新”**，就可以看到新创建的私有网络和子网了。

# 核心概念
以下是京瞳AIoT应用管理平台（AIoT CV）帮助文档中使用到的概念及其解释，请参考。
...
| 资源 | 指使用京瞳AIoT应用管理平台所需资源，是用户使用京瞳AIoT应用管理平台的基本单位。各个用户创建的资源相互独立，资源隔离。|
...
| 服务 | 可被外部调用的一系列的指令和方法。相比于设置属性，服务可以提供更复杂的业务逻辑。如：执行某一任务。 |
